# Building and Maintaining s3g RNBO CLAP

This is the technical reference for RNBO authors and wrapper developers. End
users installing a prebuilt CLAP should start with the project
[README](../README.md).
...
native C++ `s3g-dsp` repo while preserving the same REAPER workflow and minimal
grayscale GUI language on both platforms.
...
the base canvas height accounts for every tab row and parameter row. The `RAND`
button randomizes exposed RNBO parameters from the wrapper side, and the
adjacent `DEV` control sets how far the current parameter values
